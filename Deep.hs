...
toRegion (Transform transformation r) = S.applyTransformation (toTransformation transformation) (toRegion r)
...
-- Functia optimizeTransformations optimizeaza transformarile aplicate unei regiuni.
optimizeTransformations :: RegionAST -> RegionAST
optimizeTransformations region = case region of
    -- Daca regiunea este o transformare, descompunem transformarea in componente,
    -- le combinam si aplicam optimizarea pe regiunea transformata.
    Transform t r -> let transformations = decomposeTransformation t
                     ...
                     in case optimizedRegion of
                        -- Daca regiunea optimizata este si ea o transformare,
                        -- combinam toate transformarile intr-una singura.
                         Transform t' r' -> let transformations' = decomposeTransformation t'
                                                allTransformations = transformations ++ transformations'
                                                fusedAllTransformations = fuseTransformations allTransformations
                                            in Transform (combineTransformations fusedAllTransformations) r'
                        -- Altfel, aplicam transformarea optimizata pe regiunea optimizata.
                         _ -> Transform (combineTransformations fusedTransformations) optimizedRegion
    -- Pentru uniune si intersectie, aplicam optimizarea pe ambele regiuni.
    Union r1 r2 -> let optimizedR1 = optimizeTransformations r1
                   ...
                   in case (optimizedR1, optimizedR2) of
                       -- Daca ambele regiuni optimizate sunt transformari cu aceeasi transformare,
                       -- putem scoate transformarea in afara uniunii/intersectiei.
                       (Transform t1 r1', Transform t2 r2') | t1 == t2 -> Transform t1 (Union r1' r2')
                       -- Altfel, pastram uniunea/intersectia ca atare.
    ...
    -- Pentru complement, aplicam optimizarea pe regiune si, daca rezultatul este o transformare,
    -- scoatem transformarea in afara complementului.
    ...
    -- Pentru alte tipuri de regiuni, nu facem nicio optimizare.
    _ -> region
