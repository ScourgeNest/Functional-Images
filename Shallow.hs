...
plot width height region =
    init $ unlines [row (fromIntegral y) | y <- [height, height - 1 .. -height]]
  where
    row y = [if inside (fromIntegral x, y) region then '*' else '.' | x <- [-width .. width]]
...
promoteBinary f g1 g2 = \point -> f (g1 point) (g2 point)
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea leneșă în funcția circles generează regiunea doar când este necesar,
             economisind resurse, și se oprește când găsește punctul, asigurând eficiență maximă.
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: Când se verifică apartenența unui punct care nu aparține regiunii infiniteCircles,
             procesul intră într-o buclă infinită. Deoarece regiunea este definită ca o uniune infinită de cercuri,
             iar aceasta include o transformare care adaugă cercuri noi în continuu,
             verificarea nu va ajunge niciodată la o concluzie și va rula în mod infinit.
-}
...
bfs start expand = 
    let parse [] _ = []
        parse ((state, distance):xs) visited = 
            if S.member state visited
            then parse xs visited
            else (state, distance) : parse (xs ++ [(neighbour, distance + 1) | neighbour <- expand state]) (S.insert state visited)
    in 
    parse [(start, 0)] S.empty
...
regionAvoidingBfs start region = bfs start (expand region)
  where
    expand region point = [p | p <- [north point, east point, south point, west point], not $ inside p region]
    north (x, y) = (x, y + 1)
    east (x, y) = (x + 1, y)
    south (x, y) = (x, y - 1)
    west (x, y) = (x - 1, y)
